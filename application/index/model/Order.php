...
class Order extends Model{
    ...
    //接口地址: https://yp.dxshuju.com/app/public/index/order/getOrderInfo
    //条件       bis_id(店铺id)    type(类型)  telephone(用户手机号)
    //传输方式  $_POST
    //transport_type    使用类型1-普通类型，即使用模板 2-一口价      ykj_price   一口价
    // order_status 订单状态(1-订单未处理 2-订单已确认，款未到 3-款已到，正在备货 4-货已发出 5-货已收到，谢谢您的定购 )
    //关联表           store_main_orders   订单表      store_post_mode     送货方式表
    ...
    public function getOrderInfo($param){
        ...
        if($type == 2){
            $con = "and main.order_status = 1";//1-订单未处理
        }elseif($type == 3){
            $con = "and main.order_status = 2";//2-订单已确认，款未到
        }elseif($type == 4){
            $con = "and main.order_status = 3";//3-款已到，正在备货
        }elseif($type == 5){
            $con = "and main.order_status = 4";//4-货已发出
        }elseif($type == 6){
            $con = "and main.order_status = 5";//5-货已收到，谢谢您的定购
        }else{
            $con = "";
        }
        $where .= $con;
        ...
        $result = array();
        foreach($res as $val){
            $result[$index]['order_id'] = $val['order_id'];//订单id
            $result[$index]['order_no'] = $val['order_no'];//订单号
            $result[$index]['amount'] = $val['total_amount'];//金额
            $result[$index]['status'] = $val['order_status'];//物流状态
            ...
            $result[$index]['status_text'] = $status_text;  //物流状态
            $result[$index]['pro_info'] = $this->getSubOrderInfo($val['order_id']);//商品信息
            $result[$index]['post_code'] = $val['post_code'];//快递简称
            $result[$index]['express_no'] = $val['express_no'];//快递单号
            $result[$index]['post_mode'] = $val['post_mode'];//快递全称
            $index ++;
        }
    ...
    public function getOrderInfoMulti($param){
        ...
        $res = Db::table('store_main_orders')
            ->alias('main')
            ->field('main.id as order_id,main.bis_id,
                    bis.bis_name,bis.thumb,
                    main.order_no,main.total_amount,main.order_status,main.express_no,
                    mode.post_code,mode.post_mode,main.jifen')
            ->join('store_post_mode mode','main.mode = mode.id','LEFT')
    ...
    //接口地址: https://yp.dxshuju.com/app/public/index/order/getOrderDetailInfo
    //条件    order_no(订单号)
    //传输方式  $_POST
    //transport_type    使用类型1-普通类型，即使用模板 2-一口价      ykj_price   一口价
    // order_status 订单状态(1-订单未处理 2-订单已确认，款未到 3-款已到，正在备货 4-货已发出 5-货已收到，谢谢您的定购 )
    ...
    public function getOrderDetailInfo($param){
        ...
        $res = Db::table('store_main_orders')->alias('main')
            ->field('main.id as order_id,main.order_no,main.total_amount,main.order_status,main.mobile,main.address,main.rec_name,main.create_time,main.pay_time,main.payment,main.pro_total_amount,main.transport_fee,main.express_no,mode.post_code,mode.post_mode,main.jifen')
            ->join('store_post_mode mode','main.mode = mode.id','LEFT')
            ->where($where)
            ->find();
        ...
        $res['payment_text'] = $payment_text;//支付类型
        $res['status_text'] = $status_text;//商品状态
    ...
    //接口地址: https://yp.dxshuju.com/app/public/index/order/makeOrder
    //条件            bis_id(店铺id)   pro_id（商品id ）
    //传输方式  $_POST
    //transport_type    使用类型1-普通类型，即使用模板 2-一口价      ykj_price   一口价
    //关联表           store_bis   店铺表     store_main_orders 主表       store_pro_config     商品配置表
    //  store_pro_images  商品图片表  store_products  商品表  store_sub_orders  副表      store_shopping_carts
    public function makeOrder($param){
        ...
        $bis_id = !empty($param['bis_id']) ? $param['bis_id'] : '';
        $rec_name = !empty($param['rec_name']) ? $param['rec_name'] : '';   //收件人姓名
        $mobile = !empty($param['mobile']) ? $param['mobile'] : '';     //手机号
        $address = !empty($param['address']) ? $param['address'] : '';     //收件地址
        $total_amount = !empty($param['total_amount']) ? $param['total_amount'] : '';   //总金额
        $remark = !empty($param['remark']) ? $param['remark'] : ''; //备注
        $pro_amount = !empty($param['pro_amount']) ? $param['pro_amount'] : ''; //金额
        ...
        $pro_info= !empty($param['pro_info']) ? $param['pro_info'] : '';   //商品配置表的商品id    pro_id
//        //补全店铺id格式
        if($bis_id < 10){
...
//        //向主表添加数据
        ...
        $sub_data = array();
        $cart_ids = '';
        $where = "carts.bis_id = $bis_id and carts.cart_type = 1 and carts.status = 1 and con.status = 1 and img.status = 1 and carts.selected = 1";
        $pro_res[] = Db::table('store_shopping_carts')
            ->alias('carts')
            ->field('carts.id as cart_id,con.id as pro_id,pro.p_name,
            con.con_content1,con_content2,con.price as associator_price,
            carts.count,carts.selected,img.thumb,pro.rec_rate')
        ...
        foreach($pro_res as $val){
            $pro_res['cart_id'] = $val['cart_id'];  //购物车id
            $pro_res['pro_id'] = $val['pro_id'];    //商品id
            $pro_res['count'] = $val['count'];      //数量
            $pro_res['rec_rate'] = $val['rec_rate'];
            $pro_res['unit_price'] = $val['associator_price'];  //单价
            $pro_res['amount'] = $val['count'] * $val['associator_price'];  // 总价
            $pro_res['rec_amount'] = ($val['count'] * $val['associator_price']) * $val['rec_rate']; // 优惠价


        }
        //设置副订单表字段
        $temp_sub_data = [
            'main_id' => $main_res, //主订单id
            'pro_id'  => $pro_res['pro_id'],    //商品id
            'count'  => $pro_res['count'],  //购买数量
            'rec_rate'  => $pro_res['rec_rate'],
            'unit_price'   => $pro_res['unit_price'],   //单价
            'amount'  =>  $pro_res['amount'],   //金额小计
            'rec_amount'  => $pro_res['rec_amount']
        ];
    ...
    public function getSubOrderInfo($main_id){
        ...
        $res = Db::table('store_sub_orders')->alias('sub')
            ->field('pro.p_name,img.thumb,con.con_content1,con.con_content2,count,unit_price,amount')
